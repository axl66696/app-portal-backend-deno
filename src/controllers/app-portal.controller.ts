...
import { AppNews } from '@his/viewmodel/app-portal/index.js';
...
export class AppPortalController {
  ...
  async insertAppNews(message: JsMsg, payload: AppNews) {
    ...
    try {
      ...
      this.#appPortalService.addAppNews(payload);
      // let userIds: any = [];
      ...
      await this.#appPortalService.pubUserNews(payload);
  ...
  async insertUserNews(message: JsMsg, payload: any) {
    ...
    try {
      ...
      //   'updatedAt': new Date(),
      // };
      await this.#appPortalService.insertUserNews(payload);
      message.ack();
    } catch (error) {
  ...
  async getMyAppNews(message: Msg, _id: string) {
    ...
    // ];
    try {
      const myAppNews = await this.#appPortalService.getMyAppNews(_id);
  ...
  async modifyAppNews(message: JsMsg, appNews: AppNews) {
    ...
    //     'updatedAt': nowDate,
    //   };
      await this.#appPortalService.modiftAppNews(appNews);
      message.ack();
    } catch (error) {
